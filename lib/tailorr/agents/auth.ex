defmodule Tailorr.Agents.Auth do
  ...
  def search(config, %SearchQuery{} = query) do
    with {:ok, session_config} <- ensure_session(config) do
      do_search(session_config, query)
    end
  end
  ...
  defp extract_cookies(headers) do
    ...
    |> Enum.flat_map(fn {_, v} ->
      case v do
        list when is_list(list) -> Enum.map(list, &parse_cookie/1)
        str when is_binary(str) -> [parse_cookie(str)]
      end
    end)
    |> Map.new()
  end

  defp parse_cookie(cookie_str) when is_binary(cookie_str) do
